...
function generateCode() {
    ...
    // 1. Check if user wants a full clothing store/landing page (like Markhor)
    if (lowerPrompt.includes('cloth') || lowerPrompt.includes('markhor') || lowerPrompt.includes('jacket') || lowerPrompt.includes('shirt')) {
        
        // Dynamic price detection or fallback
        let priceMin = "1,000";
        let priceMax = "5,000";
        
        // Generate a real 10-product grid landing page layout with a white aesthetic background
    ...
    // 3. Fallback General Smart Layout using Unsplash Search Term
        ...
        generatedHTML = `<div style="max-width: 550px; margin: 30px auto; background: #ffffff; border: 1px solid #e4e4e7; border-radius: 16px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.06); font-family: system-ui, -apple-system, sans-serif;">
...
// === NORMAL & SIMPLE ENGLISH EXPLAINER (No Weird Technical Phrases) ===
...
function copyText(elementId) {
    ...
    navigator.clipboard.writeText(textEl.value).then(() => {
        textEl.blur(); // Removese highlighting/selection instantly
    });
}
